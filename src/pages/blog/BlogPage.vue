...
import { getBlogList, getUserCurrent, getUserSearchOne, getBlogLike } from '@/api/controller'
...
const likeBlog = async (blog: any) => {
  ...
  const likedInitially = blog._liked === true
  const delta = likedInitially ? -1 : 1
  // 乐观更新
  blog.praise = originalPraise + delta
  blog._liked = !likedInitially
  try {
    const res = await getBlogLike({ blogId: blog.id })
    if (res.data.code === 0) {
      // 如果后端返回最新点赞数
      if (typeof res.data.data === 'number') {
        blog.praise = res.data.data
      }
